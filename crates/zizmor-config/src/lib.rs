...
const DEPENDABOT_COOLDOWN: &str = "dependabot-cooldown";
const FORBIDDEN_USES: &str = "forbidden-uses";
const KNOWN_VULNERABLE_ACTIONS: &str = "known-vulnerable-actions";
const SECRETS_OUTSIDE_ENV: &str = "secrets-outside-env";
const UNPINNED_USES: &str = "unpinned-uses";
...
#[serde(transparent)]
pub struct ForbiddenUsesConfig(
    ...
    #[serde(with = "yaml_serde::with::singleton_map")] pub ForbiddenUsesConfigInner,
);
...
pub enum ForbiddenUsesConfigInner {
...
#[serde(default)]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
pub struct SecretsOutsideEnvConfig {
...
#[derive(Clone, Debug)]
pub struct SecretsOutsideEnvPolicy {
    /// List of secret names excluded from the audit
...
#[derive(Clone, Debug)]
pub struct UnpinnedUsesPolicies {
...
#[derive(Error, Debug)]
pub enum UnpinnedUsesConfigError {
...
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
#[serde(rename_all = "kebab-case", deny_unknown_fields)]
pub struct KnownVulnerableActionsConfig {
...
pub struct Config {
    raw: RawConfig,
    pub dependabot_cooldown_config: DependabotCooldownConfig,
    pub forbidden_uses_config: Option<ForbiddenUsesConfig>,
    pub secrets_outside_env_policy: SecretsOutsideEnvPolicy,
    pub unpinned_uses_policies: UnpinnedUsesPolicies,
    pub known_vulnerable_actions_config: KnownVulnerableActionsConfig,
}

impl Config {
    /// Loads a [`Config`] from the given contents.
    pub fn parse(contents: &str) -> Result<Self, ConfigErrorInner> {
        let raw = RawConfig::load(contents)?;

        let dependabot_cooldown_config = raw.rule_config(DEPENDABOT_COOLDOWN)?.unwrap_or_default();

        let forbidden_uses_config = raw.rule_config(FORBIDDEN_USES)?;

        let secrets_outside_env_config =
            raw.rule_config::<SecretsOutsideEnvConfig>(SECRETS_OUTSIDE_ENV)?;
        ...
        let unpinned_uses_policies = {
            if let Some(unpinned_uses_config) =
                raw.rule_config::<UnpinnedUsesConfig>(UNPINNED_USES)?
            {
        ...
        let known_vulnerable_actions_config = raw
            .rule_config::<KnownVulnerableActionsConfig>(KNOWN_VULNERABLE_ACTIONS)?
            .unwrap_or_default();
    ...
    pub fn parse_named(contents: &str, path: impl Into<String>) -> Result<Self, ConfigError> {
        Self::parse(contents).map_err(|source| ConfigError {
            path: path.into(),
            source,
        })
    }
        ...
        if let Some(root) = root {
            ...
            for candidate in CONFIG_CANDIDATES {
                ...
                if candidate_path.is_file() {
                    ...
                    return Ok(Some(Self::parse(&fs::read_to_string(&candidate_path)?)?));
                }
            }
    ...
    pub fn ignore_rules(&self, ident: &str) -> &[WorkflowRule] {
        self.raw
            .rules
            .get(ident)
            .map(|rule_config| rule_config.ignore.as_slice())
            .unwrap_or_default()
    }

    pub fn severity_remap_for(&self, ident: &str) -> Option<Severity> {
        self.raw
            .rules
            .get(ident)
