...
pub(super) struct LineRef<'a> {
    ...
    pub peeled_obj_id: Option<&'a str>,
}

impl<'a> LineRef<'a> {
    /// Turn a decoded pkt-line data packet into a `LineRef`.
    pub(super) fn parse(data: pktline::Data<'a>) -> Result<Self, LineRefError> {
...
pub(super) struct LineRefIterator<'a> {
