...
impl<'a> Packet<'a> {
    pub(super) fn data(data: &'a [u8]) -> Result<Self, PktLineError> {
        Data::try_from(data).map(Self::Data)
    }

    pub(super) fn encode(&self, dest: &mut Vec<u8>) -> Result<(), PktLineError> {
        match self {
            Packet::Data(data) => {
                let len = data.len() + 4;
    ...
    pub(super) fn decode(packet: &'a [u8]) -> Result<Self, PktLineError> {
...
pub(super) struct PacketIterator<'a> {
    data: &'a [u8],
    position: usize,
}
