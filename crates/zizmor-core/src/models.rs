...
pub fn if_is_statically_false(cond: &common::If) -> bool {
    match cond {
        common::If::Bool(b) => !*b,
        common::If::Expr(raw) => {
                    ...
                    Ok(raw_value) => {
                        let evaluation = Self::validator().evaluate(
                            &serde_json::to_value(&raw_value)
                                .map_err(|e| InputError::Syntax(e.into()))?,
                        );
                        ...
                        if evaluation.flag().valid {
                            ...
                            Err(InputError::Schema(parse_validation_errors(errors)))
                        }
                    }
                    // Syntax error.
...
pub trait StepCommon<'doc>: Locatable<'doc> + HasInputs {
