...
use crate::{models::repo_ref::RepoRef, utils::ExtractedExpr};
...
impl ActionCoordinate {
    pub fn uses_pattern(&self) -> &RepositoryUsesPattern {
        match self {
    ...
    pub fn usage<'doc>(&self, step: &impl StepCommon<'doc>) -> Option<Usage> {
...
impl ControlExpr {
    ...
    pub fn any(exprs: impl IntoIterator<Item = Self>) -> Self {
    ...
    pub fn eval(&self, with: &IndexMap<String, EnvValue>) -> ControlEvaluation {
        match self {
            Self::Single {
                toggle,
                field_name,
