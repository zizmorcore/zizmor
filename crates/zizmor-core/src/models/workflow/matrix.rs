...
pub struct Expansions<'doc>(Vec<Expansion<'doc>>);

impl<'doc> Expansions<'doc> {
    pub fn new(matrix: &Matrix<'doc>) -> Self {
        Self::expand_values(matrix)
    }

    pub fn iter(&self) -> impl Iterator<Item = &Expansion<'doc>> {
        self.0.iter()
    }
...
impl<'doc> Matrix<'doc> {
    ...
    pub fn expansions(&self) -> Expansions<'doc> {
        Expansions::new(self)
    }

    /// Checks whether some expanded path leads to an expression
    pub fn expands_to_static_values(&self, context: &Context) -> bool {
