...
impl Action {
    ...
    pub fn docker<'doc>(&'doc self) -> Option<DockerAction<'doc>> {
    ...
    pub fn steps(&self) -> Option<CompositeSteps<'_>> {
    ...
    pub fn conditions(&self) -> impl Iterator<Item = (&common::If, SymbolicLocation<'_>)> {
        self.steps()
            .into_iter()
            .flatten()
...
impl<'a> CompositeStep<'a> {
    pub fn new(index: usize, inner: &'a action::Step, parent: &'a Action) -> Self {
        Self {
            index,
            inner,
            parent,
