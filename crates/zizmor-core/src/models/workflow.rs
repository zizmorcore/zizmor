...
impl Workflow {
    ...
    pub fn from_string(contents: String, key: InputKey) -> Result<Self, InputError> {
    ...
    pub fn has_pull_request_target(&self) -> bool {
        match &self.on {
    ...
    pub fn has_workflow_run(&self) -> bool {
        match &self.on {
    ...
    pub fn has_single_trigger(&self) -> bool {
        match &self.on {
    ...
    pub fn is_reusable_only(&self) -> bool {
...
impl<'doc> NormalJob<'doc> {
    pub fn new(id: &'doc str, inner: &'doc job::NormalJob, parent: &'doc Workflow) -> Self {
        Self { id, inner, parent }
    }

    /// This job's matrix, if it has one.
    pub fn matrix(&self) -> Option<Matrix<'doc>> {
        Matrix::new(self)
    }

    /// An iterator of this job's constituent [`Step`]s.
    pub fn steps(&self) -> Steps<'doc> {
        Steps::new(self)
    }
    ...
    pub fn runner_default_shell(&self) -> Option<&'static str> {
        match &self.runs_on {
    ...
    pub fn conditions(&self) -> impl Iterator<Item = (&'doc common::If, SymbolicLocation<'doc>)> {
...
#[derive(Clone)]
pub struct ReusableWorkflowCallJob<'doc> {
    /// The job's unique ID (i.e., its key in the workflow's `jobs:` block).
    id: &'doc str,
    /// The underlying job.
...
pub trait JobCommon<'doc>: Locatable<'doc> {
    /// The job's unique ID (i.e., its key in the workflow's `jobs:` block).
    fn id(&self) -> &'doc str;
...
impl<'doc> Step<'doc> {
    ...
    pub fn r#if(&self) -> Option<&'doc common::If> {
        match self.inner {
    ...
    pub fn env(&self) -> &'doc LoE<common::Env> {
        match self.inner {
    ...
    pub fn job(&self) -> &NormalJob<'doc> {
        &self.parent
    }

    /// Returns this step's (grand)parent [`Workflow`].
    pub fn workflow(&self) -> &'doc Workflow {
        self.parent.parent()
    }
    ...
    pub fn shell(&self) -> Option<(&str, SymbolicLocation<'doc>)> {
