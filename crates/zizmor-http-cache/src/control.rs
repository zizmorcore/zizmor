...
pub(crate) struct CacheControl {
    ...
    pub(crate) max_stale_seconds: Option<u64>,
    /// <https://www.rfc-editor.org/rfc/rfc9111.html#name-min-fresh>
    pub(crate) min_fresh_seconds: Option<u64>,
    ...
    pub(crate) s_maxage_seconds: Option<u64>,
    ...
    fn from_iter<T: IntoIterator<Item = CacheControlDirective>>(it: T) -> Self {
        ...
        for directive in it {
            ...
            match &*directive.name {
                ...
                "max-age" => match parse_int(&directive.value) {
                ...
                "max-stale" => {
                    ...
                    if directive.value.is_empty() {
                        ...
                        match parse_int(&directive.value) {
                            None => cc.must_revalidate = true,
                            Some(seconds) => cc.max_stale_seconds = Some(seconds),
                        }
                    }
                }
                "min-fresh" => match parse_int(&directive.value) {
                    None => cc.must_revalidate = true,
                ...
                "s-maxage" => match parse_int(&directive.value) {
                    None => cc.must_revalidate = true,
...
/// `zizmor`. For example, we don't need to handle quoted values at all
/// since we either don't use or care about values that require quoting. With
/// that said, the parser handles these because it wasn't that much extra work
/// to do so and just generally seemed like good sense. (If we didn't handle
/// them and parsed them incorrectly, that might mean parsing subsequent
/// directives that we do care about incorrectly.)
///
...
impl<'b, B: 'b + ?Sized + AsRef<[u8]>, I: Iterator<Item = &'b B>> CacheControlParser<'b, I> {
    ...
    fn emit_directive(
        ...
        if self.seen.insert(directive.name.clone()) {
            Some(directive)
        } else {
    ...
    fn emit_revalidation(&mut self) -> Option<CacheControlDirective> {
        // If we've already emitted a must-revalidate directive, then don't do
        // it again.
        self.seen
            .insert("must-revalidate".to_string())
            .then(CacheControlDirective::must_revalidate)
    }
}
        ...
        loop {
            ...
            while !self.cur.is_empty() {
                ...
                let Some(mut name) = self.parse_token() else {
                    ...
                    if invalid && let Some(directive) = self.emit_revalidation() {
                ...
                if let Some(directive) = self.emit_directive(directive) {
                    return Some(directive);
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn cache_control_valueless_max_stale() {
        let cache_control: CacheControl = CacheControlParser::new(["max-stale"]).collect();

        assert_eq!(cache_control.max_stale_seconds, Some(u64::MAX));
    }
}
