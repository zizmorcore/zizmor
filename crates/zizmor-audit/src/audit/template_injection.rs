            ...
            for (context, origin) in parsed.dataflow_contexts() {
                ...
                match context.as_pattern().as_deref() {
                    Some(context_pattern) => {
                        ...
                        match capability_from_context(context_pattern) {
...
mod tests {
    ...
    use crate::audit::template_injection::{
        Capability, TemplateInjection, capability_from_context,
    };

    #[test]
    fn test_capability_from_context() {
        assert!(matches!(
            capability_from_context("github.event.workflow_run.triggering_actor.login"),
            Some(Capability::Arbitrary)
        ));
        assert!(matches!(
            capability_from_context("github.event.answer.user.gists_url"),
            Some(Capability::Structured)
        ));
        assert!(matches!(
            capability_from_context("github.event.type.is_enabled"),
            Some(Capability::Fixed)
        ));
        assert!(matches!(
            capability_from_context("runner.arch"),
            Some(Capability::Fixed)
        ));
    }
